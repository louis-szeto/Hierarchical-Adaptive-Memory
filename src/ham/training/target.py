...
training cost to reach a target accuracy. "Cost" is the triple (optimizer steps,
training tokens, wall-clock seconds).
"""
...
def _leg_curve(checkpoints: Iterable[CheckpointEval]) -> list[tuple]:
    """Ordered (step, tokens_seen, wall_clock_s, accuracy) for one leg."""
    out = []
    for ckpt in checkpoints:
        out.append((ckpt.step, ckpt.tokens_seen, ckpt.wall_clock_s, ckpt.accuracy()))
    return out
...
def parity_target(checkpoints: dict, parity_with: str,
    ...
    if not curve:
        return 0.0
    peak = max(acc for _, _, _, acc in curve)
...
def cost_to_target(checkpoints: list[CheckpointEval], leg: str, target: float,
    ...
    the token / wall-clock cost between the last-below and first-at-or-above
    checkpoints for a smoother estimate. Returns ``reached=False`` honestly if
    the leg never crosses the target.
    ...
    hit_idx = None
    for i, (_s, _t, _w, acc) in enumerate(curve):
        if acc >= target:
            hit_idx = i
            break
    if hit_idx is None:
        return _cost_result(reached=False, final_accuracy=curve[-1][3],
                            max_accuracy=max(a for _, _, _, a in curve))
    step, tokens, wall, acc = curve[hit_idx]
    if interpolate and hit_idx > 0:
        prev_step, prev_tokens, prev_wall, prev_acc = curve[hit_idx - 1]
        denom = acc - prev_acc
        ...
        wall = prev_wall + frac * (wall - prev_wall)
        step = prev_step + frac * (step - prev_step)
    return _cost_result(
        reached=True, optimizer_steps=step, training_tokens=tokens,
        wall_clock_s=wall, accuracy_at_target=acc,
        final_accuracy=curve[-1][3], max_accuracy=max(a for _, _, _, a in curve))


def _cost_result(reached: bool, optimizer_steps: float | None = None,
                 training_tokens: float | None = None, wall_clock_s: float | None = None,
    ...
    return {
        ...
        "wall_clock_to_target_s": wall_clock_s,
